...
class FormTool(UniqueObject, SimpleItem):
    ...
    form_action_key = 'form_action'         # the action for the form
        ...
        if id:
            if bad_id(id):
                ...
                if context.__class__ == PendingCreate:
                    ...
                    checkForCollision = 1
                    container = context.getParentNode().getParentNode()
                else:
    ...
    def __bobo_traverse__(self, REQUEST, name):
        ...
        if not do_validate:
            self.log('would go to %s.%s' % (str(aq_parent(self)), name))
...
class FormValidator(SimpleItem):
    ...
    def __call__(self, REQUEST, **kw):
        ...
        REQUEST.set(FormTool.form_action_key, REQUEST.get('URL0'))
        ...
        if self.do_validate:
            ...
            self.log('instead going to to %s.%s' % (str(aq_parent(self)), self.form))
...
#        """ """
#        return self.__call__(REQUEST, **kw)
