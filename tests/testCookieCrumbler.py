...
class TestCookieCrumbler(PloneTestCase.FunctionalTestCase):
    ...
    def afterSetUp(self):
        self.server_url = self.app.absolute_url()
        ...
        self.cookie = base64.encodestring(self.auth_info)
    ...
    def testAutoLoginPage(self):
        ...
        self.failUnless(location.startswith(self.portal_url + '/require_login'))

        response = self.publish(location[len(self.server_url):])
        self.assertEqual(response.getStatus(), 302)
    ...
    def testInsufficientPrivileges(self):
        ...
        response = self.publish(location[len(self.server_url):], extra={'__ac': self.cookie})
        self.assertEqual(response.getStatus(), 302)
