...
class TestPUTWithMetadata(PloneTestCase.FunctionalTestCase):
    # PUT objects into Plone while crafting a special url that will
    # turn the traversed path into an action to be applied to the
    # content being PUT.
    # See PLIP 187.
    ...
    def testPUTDocument(self):
        ...
        response = self.publish(
            self.folder_path + ('/++props++subject:plone, plip, love'
                                '++description:We can work it out'
                                '++title:The Beagles/new_html'),
                                env={'CONTENT_TYPE': 'text/html'},
                                request_method='PUT',
                                stdin=StringIO(),
                                basic=self.basic_auth)
        ...
        self.failUnless('new_html' in self.folder.objectIds())
        self.assertEqual(self.folder.new_html.portal_type, 'Document')
        self.assertEqual(self.folder.new_html.Subject(), 
                         ('plone', 'plip', 'love'))
        self.assertEqual(self.folder.new_html.Title(), 
                         'The Beagles')
        self.assertEqual(self.folder.new_html.Description(), 
                         'We can work it out')
...
def test_suite():
    ...
    suite.addTest(makeSuite(TestPUTWithMetadata))
