...
class RecycleBinView(BrowserView):
    ...
    def restore_items(self):
        ...
        if not selected_items:
            message = u"No items selected for restoration."
    ...
    def delete_items(self):
        ...
        if not selected_items:
            message = u"No items selected for deletion."
            IStatusMessage(self.request).addStatusMessage(message, type="info")
            return
    ...
    def restore_item(self):
        ...
        if target_path:
            try:
                ...
                self.request.response.redirect(f"{self.context.absolute_url()}/recyclebin/item/{self.item_id}")
                return
