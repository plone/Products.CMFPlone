...
},CstyleBehaviour.$matchTokenType=function(token,types){return types.indexOf(token.type||token)>-1},CstyleBehaviour.recordAutoInsert=function(editor,session,bracket){var cursor=editor.getCursorPosition(),line=session.doc.getLine(cursor.row);this.isAutoInsertedClosing(cursor,line,context.autoInsertedLineEnd[0])||(context.autoInsertedBrackets=0),context.autoInsertedRow=cursor.row,context.autoInsertedLineEnd=bracket+line.substr(cursor.column),context.autoInsertedBrackets++},CstyleBehaviour.recordMaybeInsert=function(editor,session,bracket){var cursor=editor.getCursorPosition(),line=session.doc.getLine(cursor.row);this.isMaybeInsertedClosing(cursor,line)||(context.maybeInsertedBrackets=0),context.maybeInsertedRow=cursor.row,context.maybeInsertedLineStart=line.substr(0,cursor.column)+bracket,context.maybeInsertedLineEnd=line.substr(cursor.column),context.maybeInsertedBrackets++},CstyleBehaviour.isAutoInsertedClosing=function(cursor,line,bracket){return context.autoInsertedBrackets>0&&cursor.row===context.autoInsertedRow&&bracket===context.autoInsertedLineEnd[0]&&line.substr(cursor.column)===context.autoInsertedLineEnd},CstyleBehaviour.isMaybeInsertedClosing=function(cursor,line){return context.maybeInsertedBrackets>0&&cursor.row===context.maybeInsertedRow&&line.substr(cursor.column)===context.maybeInsertedLineEnd&&line.substr(0,cursor.column)==context.maybeInsertedLineStart},CstyleBehaviour.popAutoInsertedClosing=function(){context.autoInsertedLineEnd=context.autoInsertedLineEnd.substr(1),context.autoInsertedBrackets--},CstyleBehaviour.clearMaybeInsertedClosing=function(){context&&(context.maybeInsertedBrackets=0,context.maybeInsertedRow=-1)},oop.inherits(CstyleBehaviour,Behaviour),exports.CstyleBehaviour=CstyleBehaviour}),define("ace/mode/behaviour/css",["require","exports","module","ace/lib/oop","ace/mode/behaviour","ace/mode/behaviour/cstyle","ace/token_iterator"],function(require,exports){var oop=require("../../lib/oop"),CstyleBehaviour=(require("../behaviour").Behaviour,require("./cstyle").CstyleBehaviour),TokenIterator=require("../../token_iterator").TokenIterator,CssBehaviour=function(){this.inherit(CstyleBehaviour),this.add("colon","insertion",function(state,action,editor,session,text){if(":"===text){var cursor=editor.getCursorPosition(),iterator=new TokenIterator(session,cursor.row,cursor.column),token=iterator.getCurrentToken();if(token&&token.value.match(/\s+/)&&(token=iterator.stepBackward()),token&&"support.type"===token.type){var line=session.doc.getLine(cursor.row),rightChar=line.substring(cursor.column,cursor.column+1);if(":"===rightChar)return{text:"",selection:[1,1]};if(!line.substring(cursor.column).match(/^\s*;/))return{text:":;",selection:[1,1]}}}}),this.add("colon","deletion",function(state,action,editor,session,range){var selected=session.doc.getTextRange(range);if(!range.isMultiLine()&&":"===selected){var cursor=editor.getCursorPosition(),iterator=new TokenIterator(session,cursor.row,cursor.column),token=iterator.getCurrentToken();if(token&&token.value.match(/\s+/)&&(token=iterator.stepBackward()),token&&"support.type"===token.type){var line=session.doc.getLine(range.start.row),rightChar=line.substring(range.end.column,range.end.column+1);if(";"===rightChar)return range.end.column++,range}}}),this.add("semicolon","insertion",function(state,action,editor,session,text){if(";"===text){var cursor=editor.getCursorPosition(),line=session.doc.getLine(cursor.row),rightChar=line.substring(cursor.column,cursor.column+1);if(";"===rightChar)return{text:"",selection:[1,1]}}})};oop.inherits(CssBehaviour,CstyleBehaviour),exports.CssBehaviour=CssBehaviour}),define("ace/mode/folding/cstyle",["require","exports","module","ace/lib/oop","ace/range","ace/mode/folding/fold_mode"],function(require,exports){var oop=require("../../lib/oop"),Range=require("../../range").Range,BaseFoldMode=require("./fold_mode").FoldMode,FoldMode=exports.FoldMode=function(commentRegex){commentRegex&&(this.foldingStartMarker=new RegExp(this.foldingStartMarker.source.replace(/\|[^|]*?$/,"|"+commentRegex.start)),this.foldingStopMarker=new RegExp(this.foldingStopMarker.source.replace(/\|[^|]*?$/,"|"+commentRegex.end)))};oop.inherits(FoldMode,BaseFoldMode),function(){this.foldingStartMarker=/(\{|\[)[^\}\]]*$|^\s*(\/\*)/,this.foldingStopMarker=/^[^\[\{]*(\}|\])|^[\s\*]*(\*\/)/,this.getFoldWidgetRange=function(session,foldStyle,row,forceMultiline){var line=session.getLine(row),match=line.match(this.foldingStartMarker);if(match){var i=match.index;if(match[1])return this.openingBracketBlock(session,match[1],row,i);var range=session.getCommentFoldRange(row,i+match[0].length,1);return range&&!range.isMultiLine()&&(forceMultiline?range=this.getSectionRange(session,row):"all"!=foldStyle&&(range=null)),range}if("markbegin"!==foldStyle){var match=line.match(this.foldingStopMarker);if(match){var i=match.index+match[0].length;return match[1]?this.closingBracketBlock(session,match[1],row,i):session.getCommentFoldRange(row,i,-1)}}},this.getSectionRange=function(session,row){var line=session.getLine(row),startIndent=line.search(/\S/),startRow=row,startColumn=line.length;row+=1;for(var endRow=row,maxRow=session.getLength();++row<maxRow;){line=session.getLine(row);var indent=line.search(/\S/);if(-1!==indent){if(startIndent>indent)break;var subRange=this.getFoldWidgetRange(session,"all",row);if(subRange){if(subRange.start.row<=startRow)break;if(subRange.isMultiLine())row=subRange.end.row;else if(startIndent==indent)break}endRow=row}}return new Range(startRow,startColumn,endRow,session.getLine(endRow).length)}}.call(FoldMode.prototype)}),define("ace/mode/css",["require","exports","module","ace/lib/oop","ace/mode/text","ace/mode/css_highlight_rules","ace/mode/matching_brace_outdent","ace/worker/worker_client","ace/mode/behaviour/css","ace/mode/folding/cstyle"],function(require,exports){var oop=require("../lib/oop"),TextMode=require("./text").Mode,CssHighlightRules=require("./css_highlight_rules").CssHighlightRules,MatchingBraceOutdent=require("./matching_brace_outdent").MatchingBraceOutdent,WorkerClient=require("../worker/worker_client").WorkerClient,CssBehaviour=require("./behaviour/css").CssBehaviour,CStyleFoldMode=require("./folding/cstyle").FoldMode,Mode=function(){this.HighlightRules=CssHighlightRules,this.$outdent=new MatchingBraceOutdent,this.$behaviour=new CssBehaviour,this.foldingRules=new CStyleFoldMode};oop.inherits(Mode,TextMode),function(){this.foldingRules="cStyle",this.blockComment={start:"/*",end:"*/"},this.getNextLineIndent=function(state,line,tab){var indent=this.$getIndent(line),tokens=this.getTokenizer().getLineTokens(line,state).tokens;if(tokens.length&&"comment"==tokens[tokens.length-1].type)return indent;var match=line.match(/^.*\{\s*$/);return match&&(indent+=tab),indent},this.checkOutdent=function(state,line,input){return this.$outdent.checkOutdent(line,input)},this.autoOutdent=function(state,doc,row){this.$outdent.autoOutdent(doc,row)},this.createWorker=function(session){var worker=new WorkerClient(["ace"],"ace/mode/css_worker","Worker");return worker.attachToDocument(session.getDocument()),worker.on("csslint",function(e){session.setAnnotations(e.data)}),worker.on("terminate",function(){session.clearAnnotations()}),worker},this.$id="ace/mode/css"}.call(Mode.prototype),exports.Mode=Mode}),define("ace-mode-css",function(){}),define("mockup-patterns-texteditor",["jquery","mockup-patterns-base","mockup-utils","ace","ace-theme-monokai","ace-mode-text","ace-mode-javascript","ace-mode-css"],function($,Base,utils){var AcePattern=Base.extend({name:"texteditor",trigger:".pat-texteditor",defaults:{theme:null,mode:"text",width:500,height:200,tabSize:4,softTabs:!0,wrapMode:!1,showGutter:!0,showPrintMargin:!1,readOnly:!1},init:function(){var self=this;if(!window.ace)return void setTimeout(function(){self.init()},200);var ace=window.ace,id=utils.setId(self.$el);self.$wrapper=$("<div/>").css({height:self.options.height+25,width:self.options.width,position:"relative"}),self.$el.wrap(self.$wrapper),self.$el.css({width:self.options.width,height:self.options.height,position:"absolute"}),self.editor=ace.edit(id),self.options.theme&&self.setTheme(self.options.theme),self.editor.getSession().setMode("ace/mode/"+self.options.mode),self.editor.getSession().setTabSize(parseInt(self.options.tabSize,10)),self.editor.getSession().setUseSoftTabs(utils.bool(self.options.softTabs)),self.editor.getSession().setUseWrapMode(utils.bool(self.options.wrapMode)),self.editor.renderer.setShowGutter(utils.bool(self.options.showGutter)),self.editor.setShowPrintMargin(utils.bool(self.options.showPrintMargin)),self.editor.setReadOnly(utils.bool(self.options.readOnly))},setSyntax:function(name){var self=this,modes={js:"javascript",txt:"text",css:"css",html:"html",xml:"xml"},extension=name.substr(name.lastIndexOf(".")+1),mode=modes[extension];return void 0!==mode?(self.editor.getSession().setMode("ace/mode/"+mode),!0):void 0},setTheme:function(theme){var self=this;self.editor.setTheme("ace/theme/"+theme)},setText:function(data){var self=this;self.editor.setValue(data)}});return AcePattern}),define("mockup-patterns-resourceregistry-url/js/overrides",["jquery","underscore","mockup-ui-url/views/base","mockup-patterns-texteditor","mockup-utils","mockup-patterns-select2"],function($,_,BaseView,TextEditor,utils,Select2){var OverridesView=BaseView.extend({tagName:"div",className:"tab-pane overrides",editing:null,canSave:!1,template:_.template('<form class="row"><div class="col-md-12"><input class="select" type="hidden" placeholder="Select resource to override..." style="width: 100%" /></div></form><div class="row"><div class="col-md-12"><% if(view.editing){ %><p class="resource-name text-primary"><%- view.editing %></p> <div class="plone-btn-group"><button class="plone-btn plone-btn-primary plone-btn-xs disabled">Save</button> <button class="plone-btn plone-btn-default plone-btn-xs disabled">Cancel</button><button class="plone-btn plone-btn-danger plone-btn-xs">Delete customizations</button></div><% } %><div class="editor" /></div></div>'),events:{"click button.plone-btn-danger":"itemDeleted","click button.plone-btn-primary":"itemSaved","click button.plone-btn-default":"itemCancel"},initialize:function(options){BaseView.prototype.initialize.apply(this,[options]),this.tabView=options.tabView},serializedModel:function(){return $.extend({},{view:this},this.options)},itemSaved:function(e){e.preventDefault();var that=this;that.tabView.saveData("save-file",{filepath:that.editing,data:that.editor.editor.getValue()},function(){that.$el.find(".plone-btn-primary,.plone-btn-default").addClass("disabled")})},itemDeleted:function(e){e.preventDefault();var that=this;confirm("Are you sure you want to delete this override?")&&(that.options.data.overrides.splice(that.options.data.overrides.indexOf(that.editing),1),that.tabView.saveData("delete-file",{filepath:that.editing},function(){that.editing=null,that.render()}))},itemCancel:function(e){e.preventDefault(),this.editing=null,this.render()},customizeResource:function(resource){-1===this.options.data.overrides.indexOf(resource)&&this.options.data.overrides.push(resource),this.editing=resource,this.render()},afterRender:function(){var that=this,$select=that.$el.find(".select"),overrides=_.map(that.options.data.overrides,function(override){return{id:override,text:override,override:!0}}),resources=_.flatten(_.map(that.options.data.resources,function(resource){var base=resource.url||"";base&&(base+="/");var url,items=[];resource.js&&(url=base+resource.js,-1===overrides.indexOf(url)&&items.push({id:url,text:url}));for(var i=0;i<resource.css.length;i+=1)url=base+resource.css[i],-1===overrides.indexOf(url)&&items.push({id:url,text:url});return items})),format=function(data){return data.override?'<span class="customized">'+data.text+" - customized</span>":data.text};if(that.select2=new Select2($select,{data:overrides.concat(_.sortBy(resources,function(d){return d.id})),formatResult:format,formatSelection:format}),$select.on("change",function(){that.customizeResource($select.select2("val"))}),that.$editorContainer=that.$(".editor"),null!==that.editing){var url=that.options.data.baseUrl;"/"!==url[url.length-1]&&(url+="/"),that.tabView.loading.show(),$.ajax({url:url+that.editing+"?"+utils.generateId(),dataType:"text"}).done(function(data){var $pre=$('<pre class="pat-texteditor" />');$pre.text(data),that.$editorContainer.empty().append($pre),that.editor=new TextEditor($pre,{width:$(".editor").width(),height:500}),that.editor.setSyntax(that.editing),that.tabView.loading.hide(),that.editor.editor.on("change",function(){that.$el.find(".plone-btn-primary,.plone-btn-default").removeClass("disabled")})}).fail(function(){alert("error loading resource for editing"),that.tabView.loading.hide()})}}});return OverridesView}),define("mockup-patterns-backdrop",["jquery","mockup-patterns-base"],function($,Base){var Backdrop=Base.extend({name:"backdrop",trigger:".pat-backdrop",defaults:{zIndex:null,opacity:.8,className:"plone-backdrop",classActiveName:"plone-backdrop-active",closeOnEsc:!0,closeOnClick:!0},init:function(){var self=this;self.$backdrop=$("> ."+self.options.className,self.$el),0===self.$backdrop.size()&&(self.$backdrop=$("<div/>").hide().appendTo(self.$el).addClass(self.options.className),null!==self.options.zIndex&&self.$backdrop.css("z-index",self.options.zIndex)),self.options.closeOnEsc===!0&&$(document).on("keydown",function(e){self.$el.is("."+self.options.classActiveName)&&27===e.keyCode&&self.hide()}),self.options.closeOnClick===!0&&self.$backdrop.on("click",function(){self.$el.is("."+self.options.classActiveName)&&self.hide()})},show:function(){var self=this;self.$el.hasClass(self.options.classActiveName)||(self.emit("show"),self.$backdrop.css("opacity","0").show(),self.$el.addClass(self.options.classActiveName),self.$backdrop.animate({opacity:self.options.opacity},500),self.emit("shown"))},hide:function(){var self=this;self.$el.hasClass(self.options.classActiveName)&&(self.emit("hide"),self.$backdrop.animate({opacity:"0"},500).hide(),self.$el.removeClass(self.options.classActiveName),self.emit("hidden"))}});return Backdrop}),define("mockup-router",["jquery","underscore","backbone"],function($,_,Backbone){var regexEscape=function(s){return s.replace(/[-\/\\^$*+?.()|[\]{}]/g,"\\$&")},Router=Backbone.Router.extend({actions:[],redirects:{},addRoute:function(patternName,id,callback,context,pathExp,expReplace){void 0===_.findWhere(this.patterns,{patternName:patternName,id:id})&&this.actions.push({patternName:patternName,id:id,callback:callback,context:context,pathExp:pathExp,expReplace:expReplace});var regex=new RegExp("("+regexEscape(patternName)+":"+regexEscape(id)+")");this.route(regex,"handleRoute")},addRedirect:function(pathExp,destination){this.redirects[pathExp]=destination},handleRoute:function(pattern){var parts=pattern.split(":"),patternName=parts[0],id=parts[1],action=_.findWhere(this.actions,{patternName:patternName,id:id});action&&action.callback.call(action.context)},redirect:function(){var newPath,regex,hash,path=window.parent.location.pathname;if(_.some(this.actions,function(action){if(action.pathExp&&(regex=new RegExp(action.pathExp),path.match(regex))){hash="!/"+action.patternName+":"+action.id;var replaceWith="";return action.expReplace&&(replaceWith=action.expReplace),newPath=path.replace(regex,replaceWith),!0}},this),void 0===hash)for(var pathExp in this.redirects)if(regex=new RegExp(pathExp),path.match(regex)){hash="!/"+this.redirects[pathExp],newPath=path.replace(regex,"");break}void 0!==hash&&this._changeLocation.apply(this,[newPath,hash])},_changeLocation:function(path,hash){window.parent.location.hash=hash,window.parent.location.pathname=path},start:function(){Backbone.history.start()},reset:function(){this.actions=[]}});return new Router}),function(factory){"function"==typeof define&&define.amd?define("jquery.form",["jquery"],factory):factory("undefined"!=typeof jQuery?jQuery:window.Zepto)}(function($){function doAjaxSubmit(e){var options=e.data;e.isDefaultPrevented()||(e.preventDefault(),$(e.target).ajaxSubmit(options))}function captureSubmittingElement(e){var target=e.target,$el=$(target);if(!$el.is("[type=submit],[type=image]")){var t=$el.closest("[type=submit]");if(0===t.length)return;target=t[0]}var form=this;if(form.clk=target,"image"==target.type)if(void 0!==e.offsetX)form.clk_x=e.offsetX,form.clk_y=e.offsetY;else if("function"==typeof $.fn.offset){var offset=$el.offset();form.clk_x=e.pageX-offset.left,form.clk_y=e.pageY-offset.top}else form.clk_x=e.pageX-target.offsetLeft,form.clk_y=e.pageY-target.offsetTop;setTimeout(function(){form.clk=form.clk_x=form.clk_y=null},100)}function log(){if($.fn.ajaxSubmit.debug){var msg="[jquery.form] "+Array.prototype.join.call(arguments,"");window.console&&window.console.log?window.console.log(msg):window.opera&&window.opera.postError&&window.opera.postError(msg)}}var feature={};feature.fileapi=void 0!==$("<input type='file'/>").get(0).files,feature.formdata=void 0!==window.FormData;var hasProp=!!$.fn.prop;$.fn.attr2=function(){if(!hasProp)return this.attr.apply(this,arguments);var val=this.prop.apply(this,arguments);return val&&val.jquery||"string"==typeof val?val:this.attr.apply(this,arguments)},$.fn.ajaxSubmit=function(options){function deepSerialize(extraData){var i,part,serialized=$.param(extraData,options.traditional).split("&"),len=serialized.length,result=[];for(i=0;len>i;i++)serialized[i]=serialized[i].replace(/\+/g," "),part=serialized[i].split("="),result.push([decodeURIComponent(part[0]),decodeURIComponent(part[1])]);return result}function fileUploadXhr(a){for(var formdata=new FormData,i=0;i<a.length;i++)formdata.append(a[i].name,a[i].value);if(options.extraData){var serializedData=deepSerialize(options.extraData);for(i=0;i<serializedData.length;i++)serializedData[i]&&formdata.append(serializedData[i][0],serializedData[i][1])}options.data=null;var s=$.extend(!0,{},$.ajaxSettings,options,{contentType:!1,processData:!1,cache:!1,type:method||"POST"});options.uploadProgress&&(s.xhr=function(){var xhr=$.ajaxSettings.xhr();return xhr.upload&&xhr.upload.addEventListener("progress",function(event){var percent=0,position=event.loaded||event.position,total=event.total;event.lengthComputable&&(percent=Math.ceil(position/total*100)),options.uploadProgress(event,position,total,percent)},!1),xhr}),s.data=null;var beforeSend=s.beforeSend;return s.beforeSend=function(xhr,o){o.data=options.formData?options.formData:formdata,beforeSend&&beforeSend.call(this,xhr,o)},$.ajax(s)}function fileUploadIframe(a){function getDoc(frame){var doc=null;try{frame.contentWindow&&(doc=frame.contentWindow.document)}catch(err){log("cannot get iframe.contentWindow document: "+err)}if(doc)return doc;try{doc=frame.contentDocument?frame.contentDocument:frame.document}catch(err){log("cannot get iframe.contentDocument: "+err),doc=frame.document}return doc}function doSubmit(){function checkState(){try{var state=getDoc(io).readyState;log("state = "+state),state&&"uninitialized"==state.toLowerCase()&&setTimeout(checkState,50)}catch(e){log("Server abort: ",e," (",e.name,")"),cb(SERVER_ABORT),timeoutHandle&&clearTimeout(timeoutHandle),timeoutHandle=void 0}}var t=$form.attr2("target"),a=$form.attr2("action");form.setAttribute("target",id),(!method||/post/i.test(method))&&form.setAttribute("method","POST"),a!=s.url&&form.setAttribute("action",s.url),s.skipEncodingOverride||method&&!/post/i.test(method)||$form.attr({encoding:"multipart/form-data",enctype:"multipart/form-data"}),s.timeout&&(timeoutHandle=setTimeout(function(){timedOut=!0,cb(CLIENT_TIMEOUT_ABORT)},s.timeout));var extraInputs=[];try{if(s.extraData)for(var n in s.extraData)s.extraData.hasOwnProperty(n)&&extraInputs.push($.isPlainObject(s.extraData[n])&&s.extraData[n].hasOwnProperty("name")&&s.extraData[n].hasOwnProperty("value")?$('<input type="hidden" name="'+s.extraData[n].name+'">').val(s.extraData[n].value).appendTo(form)[0]:$('<input type="hidden" name="'+n+'">').val(s.extraData[n]).appendTo(form)[0]);s.iframeTarget||$io.appendTo("body"),io.attachEvent?io.attachEvent("onload",cb):io.addEventListener("load",cb,!1),setTimeout(checkState,15);try{form.submit()}catch(err){var submitFn=document.createElement("form").submit;submitFn.apply(form)}}finally{form.setAttribute("action",a),t?form.setAttribute("target",t):$form.removeAttr("target"),$(extraInputs).remove()}}function cb(e){if(!xhr.aborted&&!callbackProcessed){if(doc=getDoc(io),doc||(log("cannot access response document"),e=SERVER_ABORT),e===CLIENT_TIMEOUT_ABORT&&xhr)return xhr.abort("timeout"),void deferred.reject(xhr,"timeout");if(e==SERVER_ABORT&&xhr)return xhr.abort("server abort"),void deferred.reject(xhr,"error","server abort");if(doc&&doc.location.href!=s.iframeSrc||timedOut){io.detachEvent?io.detachEvent("onload",cb):io.removeEventListener("load",cb,!1);var errMsg,status="success";try{if(timedOut)throw"timeout";var isXml="xml"==s.dataType||doc.XMLDocument||$.isXMLDoc(doc);if(log("isXml="+isXml),!isXml&&window.opera&&(null===doc.body||!doc.body.innerHTML)&&--domCheckCount)return log("requeing onLoad callback, DOM not available"),void setTimeout(cb,250);var docRoot=doc.body?doc.body:doc.documentElement;xhr.responseText=docRoot?docRoot.innerHTML:null,xhr.responseXML=doc.XMLDocument?doc.XMLDocument:doc,isXml&&(s.dataType="xml"),xhr.getResponseHeader=function(header){var headers={"content-type":s.dataType};return headers[header.toLowerCase()]},docRoot&&(xhr.status=Number(docRoot.getAttribute("status"))||xhr.status,xhr.statusText=docRoot.getAttribute("statusText")||xhr.statusText);var dt=(s.dataType||"").toLowerCase(),scr=/(json|script|text)/.test(dt);if(scr||s.textarea){var ta=doc.getElementsByTagName("textarea")[0];if(ta)xhr.responseText=ta.value,xhr.status=Number(ta.getAttribute("status"))||xhr.status,xhr.statusText=ta.getAttribute("statusText")||xhr.statusText;else if(scr){var pre=doc.getElementsByTagName("pre")[0],b=doc.getElementsByTagName("body")[0];pre?xhr.responseText=pre.textContent?pre.textContent:pre.innerText:b&&(xhr.responseText=b.textContent?b.textContent:b.innerText)}}else"xml"==dt&&!xhr.responseXML&&xhr.responseText&&(xhr.responseXML=toXml(xhr.responseText));try{data=httpData(xhr,dt,s)}catch(err){status="parsererror",xhr.error=errMsg=err||status}}catch(err){log("error caught: ",err),status="error",xhr.error=errMsg=err||status}xhr.aborted&&(log("upload aborted"),status=null),xhr.status&&(status=xhr.status>=200&&xhr.status<300||304===xhr.status?"success":"error"),"success"===status?(s.success&&s.success.call(s.context,data,"success",xhr),deferred.resolve(xhr.responseText,"success",xhr),g&&$.event.trigger("ajaxSuccess",[xhr,s])):status&&(void 0===errMsg&&(errMsg=xhr.statusText),s.error&&s.error.call(s.context,xhr,status,errMsg),deferred.reject(xhr,"error",errMsg),g&&$.event.trigger("ajaxError",[xhr,s,errMsg])),g&&$.event.trigger("ajaxComplete",[xhr,s]),g&&!--$.active&&$.event.trigger("ajaxStop"),s.complete&&s.complete.call(s.context,xhr,status),callbackProcessed=!0,s.timeout&&clearTimeout(timeoutHandle),setTimeout(function(){s.iframeTarget?$io.attr("src",s.iframeSrc):$io.remove(),xhr.responseXML=null},100)}}}var el,i,s,g,id,$io,io,xhr,sub,n,timedOut,timeoutHandle,form=$form[0],deferred=$.Deferred();if(deferred.abort=function(status){xhr.abort(status)},a)for(i=0;i<elements.length;i++)el=$(elements[i]),hasProp?el.prop("disabled",!1):el.removeAttr("disabled");if(s=$.extend(!0,{},$.ajaxSettings,options),s.context=s.context||s,id="jqFormIO"+(new Date).getTime(),s.iframeTarget?($io=$(s.iframeTarget),n=$io.attr2("name"),n?id=n:$io.attr2("name",id)):($io=$('<iframe name="'+id+'" src="'+s.iframeSrc+'" />'),$io.css({position:"absolute",top:"-1000px",left:"-1000px"})),io=$io[0],xhr={aborted:0,responseText:null,responseXML:null,status:0,statusText:"n/a",getAllResponseHeaders:function(){},getResponseHeader:function(){},setRequestHeader:function(){},abort:function(status){var e="timeout"===status?"timeout":"aborted";log("aborting upload... "+e),this.aborted=1;try{io.contentWindow.document.execCommand&&io.contentWindow.document.execCommand("Stop")}catch(ignore){}$io.attr("src",s.iframeSrc),xhr.error=e,s.error&&s.error.call(s.context,xhr,e,status),g&&$.event.trigger("ajaxError",[xhr,s,e]),s.complete&&s.complete.call(s.context,xhr,e)}},g=s.global,g&&0===$.active++&&$.event.trigger("ajaxStart"),g&&$.event.trigger("ajaxSend",[xhr,s]),s.beforeSend&&s.beforeSend.call(s.context,xhr,s)===!1)return s.global&&$.active--,deferred.reject(),deferred;if(xhr.aborted)return deferred.reject(),deferred;sub=form.clk,sub&&(n=sub.name,n&&!sub.disabled&&(s.extraData=s.extraData||{},s.extraData[n]=sub.value,"image"==sub.type&&(s.extraData[n+".x"]=form.clk_x,s.extraData[n+".y"]=form.clk_y)));var CLIENT_TIMEOUT_ABORT=1,SERVER_ABORT=2,csrf_token=$("meta[name=csrf-token]").attr("content"),csrf_param=$("meta[name=csrf-param]").attr("content");csrf_param&&csrf_token&&(s.extraData=s.extraData||{},s.extraData[csrf_param]=csrf_token),s.forceSync?doSubmit():setTimeout(doSubmit,10);var data,doc,callbackProcessed,domCheckCount=50,toXml=$.parseXML||function(s,doc){return window.ActiveXObject?(doc=new ActiveXObject("Microsoft.XMLDOM"),doc.async="false",doc.loadXML(s)):doc=(new DOMParser).parseFromString(s,"text/xml"),doc&&doc.documentElement&&"parsererror"!=doc.documentElement.nodeName?doc:null},parseJSON=$.parseJSON||function(s){return window.eval("("+s+")")},httpData=function(xhr,type,s){var ct=xhr.getResponseHeader("content-type")||"",xml="xml"===type||!type&&ct.indexOf("xml")>=0,data=xml?xhr.responseXML:xhr.responseText;return xml&&"parsererror"===data.documentElement.nodeName&&$.error&&$.error("parsererror"),s&&s.dataFilter&&(data=s.dataFilter(data,type)),"string"==typeof data&&("json"===type||!type&&ct.indexOf("json")>=0?data=parseJSON(data):("script"===type||!type&&ct.indexOf("javascript")>=0)&&$.globalEval(data)),data};return deferred}if(!this.length)return log("ajaxSubmit: skipping submit process - no element selected"),this;var method,action,url,$form=this;"function"==typeof options?options={success:options}:void 0===options&&(options={}),method=options.type||this.attr2("method"),action=options.url||this.attr2("action"),url="string"==typeof action?$.trim(action):"",url=url||window.location.href||"",url&&(url=(url.match(/^([^#]+)/)||[])[1]),options=$.extend(!0,{url:url,success:$.ajaxSettings.success,type:method||$.ajaxSettings.type,iframeSrc:/^https/i.test(window.location.href||"")?"javascript:false":"about:blank"},options);var veto={};if(this.trigger("form-pre-serialize",[this,options,veto]),veto.veto)return log("ajaxSubmit: submit vetoed via form-pre-serialize trigger"),this;if(options.beforeSerialize&&options.beforeSerialize(this,options)===!1)return log("ajaxSubmit: submit aborted via beforeSerialize callback"),this;var traditional=options.traditional;void 0===traditional&&(traditional=$.ajaxSettings.traditional);var qx,elements=[],a=this.formToArray(options.semantic,elements);if(options.data&&(options.extraData=options.data,qx=$.param(options.data,traditional)),options.beforeSubmit&&options.beforeSubmit(a,this,options)===!1)return log("ajaxSubmit: submit aborted via beforeSubmit callback"),this;if(this.trigger("form-submit-validate",[a,this,options,veto]),veto.veto)return log("ajaxSubmit: submit vetoed via form-submit-validate trigger"),this;var q=$.param(a,traditional);qx&&(q=q?q+"&"+qx:qx),"GET"==options.type.toUpperCase()?(options.url+=(options.url.indexOf("?")>=0?"&":"?")+q,options.data=null):options.data=q;var callbacks=[];if(options.resetForm&&callbacks.push(function(){$form.resetForm()}),options.clearForm&&callbacks.push(function(){$form.clearForm(options.includeHidden)}),!options.dataType&&options.target){var oldSuccess=options.success||function(){};callbacks.push(function(data){var fn=options.replaceTarget?"replaceWith":"html";$(options.target)[fn](data).each(oldSuccess,arguments)})}else options.success&&callbacks.push(options.success);if(options.success=function(data,status,xhr){for(var context=options.context||this,i=0,max=callbacks.length;max>i;i++)callbacks[i].apply(context,[data,status,xhr||$form,$form])},options.error){var oldError=options.error;options.error=function(xhr,status,error){var context=options.context||this;oldError.apply(context,[xhr,status,error,$form])}}if(options.complete){var oldComplete=options.complete;options.complete=function(xhr,status){var context=options.context||this;oldComplete.apply(context,[xhr,status,$form])}}var fileInputs=$("input[type=file]:enabled",this).filter(function(){return""!==$(this).val()}),hasFileInputs=fileInputs.length>0,mp="multipart/form-data",multipart=$form.attr("enctype")==mp||$form.attr("encoding")==mp,fileAPI=feature.fileapi&&feature.formdata;log("fileAPI :"+fileAPI);var jqxhr,shouldUseFrame=(hasFileInputs||multipart)&&!fileAPI;options.iframe!==!1&&(options.iframe||shouldUseFrame)?options.closeKeepAlive?$.get(options.closeKeepAlive,function(){jqxhr=fileUploadIframe(a)}):jqxhr=fileUploadIframe(a):jqxhr=(hasFileInputs||multipart)&&fileAPI?fileUploadXhr(a):$.ajax(options),$form.removeData("jqxhr").data("jqxhr",jqxhr);for(var k=0;k<elements.length;k++)elements[k]=null;return this.trigger("form-submit-notify",[this,options]),this},$.fn.ajaxForm=function(options){if(options=options||{},options.delegation=options.delegation&&$.isFunction($.fn.on),!options.delegation&&0===this.length){var o={s:this.selector,c:this.context};return!$.isReady&&o.s?(log("DOM not ready, queuing ajaxForm"),$(function(){$(o.s,o.c).ajaxForm(options)}),this):(log("terminating; zero elements found by selector"+($.isReady?"":" (DOM not ready)")),this)}return options.delegation?($(document).off("submit.form-plugin",this.selector,doAjaxSubmit).off("click.form-plugin",this.selector,captureSubmittingElement).on("submit.form-plugin",this.selector,options,doAjaxSubmit).on("click.form-plugin",this.selector,options,captureSubmittingElement),this):this.ajaxFormUnbind().bind("submit.form-plugin",options,doAjaxSubmit).bind("click.form-plugin",options,captureSubmittingElement)},$.fn.ajaxFormUnbind=function(){return this.unbind("submit.form-plugin click.form-plugin")},$.fn.formToArray=function(semantic,elements){var a=[];if(0===this.length)return a;var form=this[0],els=semantic?form.getElementsByTagName("*"):form.elements;if(!els)return a;var i,j,n,v,el,max,jmax;for(i=0,max=els.length;max>i;i++)if(el=els[i],n=el.name,n&&!el.disabled)if(semantic&&form.clk&&"image"==el.type)form.clk==el&&(a.push({name:n,value:$(el).val(),type:el.type}),a.push({name:n+".x",value:form.clk_x},{name:n+".y",value:form.clk_y}));else if(v=$.fieldValue(el,!0),v&&v.constructor==Array)for(elements&&elements.push(el),j=0,jmax=v.length;jmax>j;j++)a.push({name:n,value:v[j]});else if(feature.fileapi&&"file"==el.type){elements&&elements.push(el);var files=el.files;if(files.length)for(j=0;j<files.length;j++)a.push({name:n,value:files[j],type:el.type});else a.push({name:n,value:"",type:el.type})}else null!==v&&"undefined"!=typeof v&&(elements&&elements.push(el),a.push({name:n,value:v,type:el.type,required:el.required}));if(!semantic&&form.clk){var $input=$(form.clk),input=$input[0];n=input.name,n&&!input.disabled&&"image"==input.type&&(a.push({name:n,value:$input.val()}),a.push({name:n+".x",value:form.clk_x},{name:n+".y",value:form.clk_y}))}return a},$.fn.formSerialize=function(semantic){return $.param(this.formToArray(semantic))},$.fn.fieldSerialize=function(successful){var a=[];return this.each(function(){var n=this.name;if(n){var v=$.fieldValue(this,successful);if(v&&v.constructor==Array)for(var i=0,max=v.length;max>i;i++)a.push({name:n,value:v[i]});else null!==v&&"undefined"!=typeof v&&a.push({name:this.name,value:v})}}),$.param(a)},$.fn.fieldValue=function(successful){for(var val=[],i=0,max=this.length;max>i;i++){var el=this[i],v=$.fieldValue(el,successful);null===v||"undefined"==typeof v||v.constructor==Array&&!v.length||(v.constructor==Array?$.merge(val,v):val.push(v))}return val},$.fieldValue=function(el,successful){var n=el.name,t=el.type,tag=el.tagName.toLowerCase();
if(void 0===successful&&(successful=!0),successful&&(!n||el.disabled||"reset"==t||"button"==t||("checkbox"==t||"radio"==t)&&!el.checked||("submit"==t||"image"==t)&&el.form&&el.form.clk!=el||"select"==tag&&-1==el.selectedIndex))return null;if("select"==tag){var index=el.selectedIndex;if(0>index)return null;for(var a=[],ops=el.options,one="select-one"==t,max=one?index+1:ops.length,i=one?index:0;max>i;i++){var op=ops[i];if(op.selected){var v=op.value;if(v||(v=op.attributes&&op.attributes.value&&!op.attributes.value.specified?op.text:op.value),one)return v;a.push(v)}}return a}return $(el).val()},$.fn.clearForm=function(includeHidden){return this.each(function(){$("input,select,textarea",this).clearFields(includeHidden)})},$.fn.clearFields=$.fn.clearInputs=function(includeHidden){var re=/^(?:color|date|datetime|email|month|number|password|range|search|tel|text|time|url|week)$/i;return this.each(function(){var t=this.type,tag=this.tagName.toLowerCase();re.test(t)||"textarea"==tag?this.value="":"checkbox"==t||"radio"==t?this.checked=!1:"select"==tag?this.selectedIndex=-1:"file"==t?/MSIE/.test(navigator.userAgent)?$(this).replaceWith($(this).clone(!0)):$(this).val(""):includeHidden&&(includeHidden===!0&&/hidden/.test(t)||"string"==typeof includeHidden&&$(this).is(includeHidden))&&(this.value="")})},$.fn.resetForm=function(){return this.each(function(){("function"==typeof this.reset||"object"==typeof this.reset&&!this.reset.nodeType)&&this.reset()})},$.fn.enable=function(b){return void 0===b&&(b=!0),this.each(function(){this.disabled=!b})},$.fn.selected=function(select){return void 0===select&&(select=!0),this.each(function(){var t=this.type;if("checkbox"==t||"radio"==t)this.checked=select;else if("option"==this.tagName.toLowerCase()){var $sel=$(this).parent("select");select&&$sel[0]&&"select-one"==$sel[0].type&&$sel.find("option").selected(!1),this.selected=select}})},$.fn.ajaxSubmit.debug=!1}),define("mockup-patterns-modal",["jquery","underscore","mockup-patterns-base","mockup-patterns-backdrop","mockup-registry","mockup-router","mockup-utils","jquery.form"],function($,_,Base,Backdrop,registry,Router,utils){var Modal=Base.extend({name:"modal",trigger:".pat-modal",createModal:null,$model:null,defaults:{width:"",height:"",margin:20,position:"center middle",triggers:[],backdrop:"body",backdropOptions:{zIndex:"1040",opacity:"0.85",className:"plone-modal-backdrop",classActiveName:"plone-backdrop-active",closeOnEsc:!0,closeOnClick:!0},title:null,titleSelector:"h1:first",buttons:'.formControls > input[type="submit"]',content:"#content",automaticallyAddButtonActions:!0,loadLinksWithinModal:!0,prependContent:".portalMessage",templateOptions:{className:"plone-modal fade",classDialog:"plone-modal-dialog",classModal:"plone-modal-content",classHeaderName:"plone-modal-header",classBodyName:"plone-modal-body",classFooterName:"plone-modal-footer",classWrapperName:"plone-modal-wrapper",classWrapperInnerName:"modal-wrapper-inner",classActiveName:"in",classPrependName:"",classContentName:"",template:'<div class="<%= options.className %>">  <div class="<%= options.classDialog %>">    <div class="<%= options.classModal %>">      <div class="<%= options.classHeaderName %>">        <a class="plone-modal-close">&times;</a>        <% if (title) { %><h2 class="plone-modal-title"><%= title %></h2><% } %>      </div>      <div class="<%= options.classBodyName %>">        <div class="<%= options.classPrependName %>"><%= prepend %></div>         <div class="<%= options.classContentName %>"><%= content %></div>      </div>      <div class="<%= options.classFooterName %>">         <% if (buttons) { %><%= buttons %><% } %>      </div>    </div>  </div></div>'},actions:{},actionOptions:{eventType:"click",target:null,ajaxUrl:null,modalFunction:null,isForm:!1,timeout:5e3,displayInModal:!0,reloadWindowOnClose:!0,error:".portalMessage.error",formFieldError:".field.error",onSuccess:null,onError:null,onFormError:null,onTimeout:null,redirectOnResponse:!1,redirectToUrl:function($action,response){var baseUrl="",reg=/<body.*data-base-url=[\"'](.*)[\"'].*/im.exec(response);return reg&&reg.length>1?baseUrl=reg[1]:(reg=/<base.*href=[\"'](.*)[\"'].*/im.exec(response),reg&&reg.length>1&&(baseUrl=reg[1])),baseUrl}},routerOptions:{id:null,pathExp:null},form:function(actions){var self=this,$modal=self.$modal;self.options.automaticallyAddButtonActions&&(actions[self.options.buttons]={}),self.options.loadLinksWithinModal&&(actions.a={}),$.each(actions,function(action,options){var actionKeys=_.union(_.keys(self.options.actionOptions),["templateOptions"]),actionOptions=$.extend(!0,{},self.options.actionOptions,_.pick(options,actionKeys));options.templateOptions=$.extend(!0,options.templateOptions,self.options.templateOptions);var patternKeys=_.union(_.keys(self.options.actionOptions),["actions","actionOptions"]),patternOptions=$.extend(!0,_.omit(options,patternKeys),self.options);$(action,$("."+options.templateOptions.classBodyName,$modal)).each(function(){var $action=$(this);$action.on(actionOptions.eventType,function(e){e.stopPropagation(),e.preventDefault(),self.loading.show(!1),null!==actionOptions.modalFunction?self[actionOptions.modalFunction]():$.nodeName($action[0],"input")||$.nodeName($action[0],"button")||options.isForm===!0?self.options.handleFormAction.apply(self,[$action,actionOptions,patternOptions]):(null!==options.ajaxUrl||$.nodeName($action[0],"a"))&&self.options.handleLinkAction.apply(self,[$action,actionOptions,patternOptions])})})})},handleFormAction:function($action,options,patternOptions){var self=this,extraData={};extraData[$action.attr("name")]=$action.attr("value");var $form;$form=$.nodeName($action[0],"form")?$action:$action.parents("form:not(.disableAutoSubmit)");var url;url=null!==options.ajaxUrl?"function"==typeof options.ajaxUrl?options.ajaxUrl.apply(self,[$action,options]):options.ajaxUrl:$action.parents("form").attr("action"),$form.on("submit",function(e){e.preventDefault()}),$form.trigger("submit"),self.loading.show(!1),$form.ajaxSubmit({timeout:options.timeout,data:extraData,url:url,error:function(xhr,textStatus,errorStatus){self.loading.hide(),"timeout"===textStatus&&options.onTimeout?options.onTimeout.apply(self,xhr,errorStatus):options.onError?options.onError(xhr,textStatus,errorStatus):console.log("error happened do something"),self.emit("formActionError",[xhr,textStatus,errorStatus])},success:function(response,state,xhr,form){return self.loading.hide(),0!==$(options.error,response).size()||0!==$(options.formFieldError,response).size()?void(options.onFormError?options.onFormError(self,response,state,xhr,form):self.redraw(response,patternOptions)):options.redirectOnResponse===!0?void(window.parent.location.href="function"==typeof options.redirectToUrl?options.redirectToUrl.apply(self,[$action,response,options]):options.redirectToUrl):(options.onSuccess&&options.onSuccess(self,response,state,xhr,form),options.displayInModal===!0?self.redraw(response,patternOptions):($action.trigger("destroy.modal.patterns"),options.reloadWindowOnClose&&self.reloadWindow()),void self.emit("formActionSuccess",[response,state,xhr,form]))}})},handleLinkAction:function($action,options,patternOptions){var url,self=this;return url=options.ajaxUrl?"function"==typeof options.ajaxUrl?options.ajaxUrl.apply(self,[$action,options]):options.ajaxUrl:$action.attr("href"),options.displayInModal===!1?void(window.parent.location.href=url):void $.ajax({url:url,error:function(xhr,textStatus,errorStatus){"timeout"===textStatus&&options.onTimeout?options.onTimeout(self.$modal,xhr,errorStatus):options.onError?options.onError(xhr,textStatus,errorStatus):console.log("error happened do something"),self.loading.hide(),self.emit("linkActionError",[xhr,textStatus,errorStatus])},success:function(response,state,xhr){self.redraw(response,patternOptions),options.onSuccess&&options.onSuccess(self,response,state,xhr),self.loading.hide(),self.emit("linkActionSuccess",[response,state,xhr])}})},render:function(options){var self=this;if(self.emit("before-render"),self.$raw){var $raw=self.$raw.clone();$("input:checked",$raw).each(function(){this.setAttribute&&this.setAttribute("checked","checked")});var tplObject={title:"",prepend:"<div />",content:"",buttons:'<div class="pattern-modal-buttons"></div>',options:options.templateOptions};if(null===options.title){var $title=$(options.titleSelector,$raw);tplObject.title=$title.html(),$(options.titleSelector,$raw).remove()}else tplObject.title=options.title;options.prependContent&&(tplObject.prepend=$("<div />").append($(options.prependContent,$raw).clone()).html(),$(options.prependContent,$raw).remove()),tplObject.content=options.content?$(options.content,$raw).html():$raw.html(),self.$modal=$(_.template(self.options.templateOptions.template,tplObject)),self.$modalDialog=$("> ."+self.options.templateOptions.classDialog,self.$modal),self.$modalContent=$("> ."+self.options.templateOptions.classModal,self.$modalDialog),$("form",self.$modal).on("keydown",function(event){13===event.keyCode&&"TEXTAREA"!==event.target.nodeName&&(event.preventDefault(),$("input[type=submit], button[type=submit], button:not(type)",this).eq(0).trigger("click"))}),$(options.buttons,self.$modal).each(function(){var $button=$(this);$button.on("click",function(e){e.stopPropagation(),e.preventDefault()}).clone().appendTo($(".pattern-modal-buttons",self.$modal)).off("click").on("click",function(e){e.stopPropagation(),e.preventDefault(),$button.trigger("click")}),$button.hide()}),self.emit("before-events-setup"),$(".plone-modal-header > a.plone-modal-close, .plone-modal-footer > a.plone-modal-close",self.$modal).off("click").on("click",function(e){e.stopPropagation(),e.preventDefault(),$(e.target).trigger("destroy.modal.patterns")}),$(".row",self.$modal).removeClass("row"),options.form&&options.form.apply(self,[options.actions]),self.$modal.addClass(self.options.templateOptions.className).on("click",function(e){e.stopPropagation(),$.nodeName(e.target,"a")&&e.preventDefault(),self.$modal.trigger("modal-click")}).on("destroy.modal.patterns",function(e){e.stopPropagation(),self.hide()}).on("resize.modal.patterns",function(e){e.stopPropagation(),e.preventDefault(),self.positionModal()}).appendTo(self.$wrapperInner),self.$modal.data("pattern-"+self.name,self),self.emit("after-render")}}},reloadWindow:function(){window.parent.location.reload()},init:function(){var self=this;if(self.backdrop=new Backdrop(self.$el.parents(self.options.backdrop),self.options.backdropOptions),self.$wrapper=$("> ."+self.options.templateOptions.classWrapperName,self.backdrop.$el),0===self.$wrapper.size()){var zIndex=null!==self.options.backdropOptions.zIndex?parseInt(self.options.backdropOptions.zIndex,10)+1:1041;self.$wrapper=$("<div/>").hide().css({"z-index":zIndex,"overflow-y":"auto",position:"fixed",height:"100%",width:"100%",bottom:"0",left:"0",right:"0",top:"0"}).addClass(self.options.templateOptions.classWrapperName).insertBefore(self.backdrop.$backdrop).on("click",function(e){e.stopPropagation(),e.preventDefault(),self.options.backdropOptions.closeOnClick&&self.backdrop.hide()})}null!==self.options.routerOptions.id&&Router.addRoute("modal",self.options.routerOptions.id,function(){this.show()},self,self.options.routerOptions.pathExp,self.options.routerOptions.expReplace),self.backdrop.on("hidden",function(){void 0!==self.$modal&&self.$modal.hasClass(self.options.templateOptions.classActiveName)&&self.hide()}),self.options.backdropOptions.closeOnEsc===!0&&$(document).on("keydown",function(e){self.$el.is("."+self.options.templateOptions.classActiveName)&&27===e.keyCode&&self.hide()}),self.$wrapperInner=$("> ."+self.options.templateOptions.classWrapperInnerName,self.$wrapper),0===self.$wrapperInner.size()&&(self.$wrapperInner=$("<div/>").addClass(self.options.classWrapperInnerName).css({position:"absolute",bottom:"0",left:"0",right:"0",top:"0"}).appendTo(self.$wrapper)),self.loading=new utils.Loading({backdrop:self.backdrop,zIndex:function(){if(!self.modalInitialized())return 10005;var zIndex=self.$modal.css("zIndex");return zIndex?parseInt(zIndex,10)+1:void 0}}),$(window.parent).resize(function(){self.positionModal()}),self.options.triggers&&$.each(self.options.triggers,function(i,item){var e=item.substring(0,item.indexOf(" ")),selector=item.substring(item.indexOf(" "),item.length);$(selector||self.$el).on(e,function(e){e.stopPropagation(),e.preventDefault(),self.show()})}),self.$el.is("a")&&(self.$el.attr("href")&&(self.options.target||"#"!==self.$el.attr("href").substr(0,1)||(self.options.target=self.$el.attr("href"),self.options.content=""),self.options.ajaxUrl||"#"===self.$el.attr("href").substr(0,1)||(self.options.ajaxUrl=self.$el.attr("href"))),self.$el.on("click",function(e){e.stopPropagation(),e.preventDefault(),self.show()})),self.initModal()},createAjaxModal:function(){var self=this;self.emit("before-ajax"),self.loading.show(),self.ajaxXHR=$.ajax({url:self.options.ajaxUrl,type:self.options.ajaxType}).done(function(response,textStatus,xhr){self.ajaxXHR=void 0,self.loading.hide(),self.$raw=$("<div />").append($(utils.parseBodyTag(response))),self.emit("after-ajax",self,textStatus,xhr),self._show()})},createTargetModal:function(){var self=this;self.$raw=$(self.options.target).clone(),self._show()},createBasicModal:function(){var self=this;self.$raw=$("<div/>").html(self.$el.clone()),self._show()},createHtmlModal:function(){var self=this,$el=$(self.options.html);self.$raw=$el,self._show()},initModal:function(){var self=this;self.createModal=self.options.ajaxUrl?self.createAjaxModal:self.options.target?self.createTargetModal:self.options.html?self.createHtmlModal:self.createBasicModal},findPosition:function(horpos,vertpos,margin,modalWidth,modalHeight,wrapperInnerWidth,wrapperInnerHeight){var absTop,absBottom,absLeft,absRight,returnpos={};return absRight=absLeft=absTop=absLeft="auto","left"===horpos?(absLeft=margin+"px",modalWidth>wrapperInnerWidth&&(absLeft="0px"),returnpos.left=absLeft):"right"===horpos?(absRight=margin+"px",modalWidth>wrapperInnerWidth&&(absRight="0px"),returnpos.right=absRight,returnpos.left="auto"):(absLeft=wrapperInnerWidth/2-modalWidth/2-margin+"px",modalWidth>wrapperInnerWidth&&(absLeft="0px"),returnpos.left=absLeft),"top"===vertpos?(absTop=margin+"px",modalHeight>wrapperInnerHeight&&(absTop="0px"),returnpos.top=absTop):"bottom"===vertpos?(absBottom=margin+"px",modalHeight>wrapperInnerHeight&&(absBottom="0px"),returnpos.bottom=absBottom,returnpos.top="auto"):(absTop=wrapperInnerHeight/2-modalHeight/2-margin+"px",modalHeight>wrapperInnerHeight&&(absTop="0px"),returnpos.top=absTop),returnpos},modalInitialized:function(){var self=this;return null!==self.$modal&&void 0!==self.$modal},positionModal:function(){var self=this;if(self.modalInitialized()){self.$modal.removeAttr("style"),self.$wrapper.parent().is("body")&&self.$wrapper.height($(window.parent).height());var margin="function"==typeof self.options.margin?self.options.margin():self.options.margin;self.$modal.css({position:"absolute",padding:margin}),self.$modalDialog.css({margin:"0",padding:"0",width:self.options.width,height:self.options.height}),self.$modalContent.css({width:self.options.width});var posopt=self.options.position.split(" "),horpos=posopt[0],vertpos=posopt[1],modalWidth=self.$modalDialog.outerWidth(!0),modalHeight=self.$modalDialog.outerHeight(!0),wrapperInnerWidth=self.$wrapperInner.width(),wrapperInnerHeight=self.$wrapperInner.height(),pos=self.findPosition(horpos,vertpos,margin,modalWidth,modalHeight,wrapperInnerWidth,wrapperInnerHeight);for(var key in pos)self.$modalDialog.css(key,pos[key])}},render:function(options){var self=this;self.emit("render"),self.options.render.apply(self,[options]),self.emit("rendered")},show:function(){var self=this;self.createModal()},_show:function(){var self=this;self.render.apply(self,[self.options]),self.emit("show"),self.backdrop.show(),self.$wrapper.show(),self.loading.hide(),self.$wrapper.parent().css("overflow","hidden"),self.$el.addClass(self.options.templateOptions.classActiveName),self.$modal.addClass(self.options.templateOptions.classActiveName),registry.scan(self.$modal),self.positionModal(),$("img",self.$modal).load(function(){self.positionModal()}),$(window.parent).on("resize.modal.patterns",function(){self.positionModal()}),self.emit("shown"),$("body").addClass("plone-modal-open")},hide:function(){var self=this;self.ajaxXHR&&self.ajaxXHR.abort(),self.emit("hide"),(!self._suppressHide||confirm(self._suppressHide))&&($(".plone-modal",self.$wrapper).size()<2&&(self.backdrop.hide(),self.$wrapper.hide(),self.$wrapper.parent().css("overflow","visible")),self.loading.hide(),self.$el.removeClass(self.options.templateOptions.classActiveName),void 0!==self.$modal&&(self.$modal.remove(),self.initModal()),$(window.parent).off("resize.modal.patterns"),self.emit("hidden"),$("body").removeClass("plone-modal-open"))},redraw:function(response,options){var self=this;self.emit("beforeDraw"),self.$modal.remove(),self.$raw=$("<div />").append($(utils.parseBodyTag(response))),self.render.apply(self,[options||self.options]),self.$modal.addClass(self.options.templateOptions.classActiveName),self.positionModal(),registry.scan(self.$modal),self.emit("afterDraw")}});return Modal}),define("mockup-patterns-resourceregistry-url/js/registry",["jquery","underscore","mockup-ui-url/views/base","mockup-utils","mockup-patterns-modal","mockup-patterns-resourceregistry-url/js/fields"],function($,_,BaseView,utils,Modal,fields){var AbstractResourceEntryView=BaseView.extend({tagName:"div",className:"resource-entry",template:_.template('<h3><%- name %></h3><div class="panel-body form-horizontal"></div>'),serializedModel:function(){return $.extend({},{name:this.name},this.options)},afterRender:function(){var self=this,$body=self.$(".panel-body");_.each(self.fields,function(field){var options=$.extend({},field,{value:self.options.data[field.name],registryData:self.options.data,containerData:self.options.containerData,resourceName:self.options.name,registryView:self.options.registryView,parent:self.options.parent});options.value||(options.value="");var View=field.view;View||(View=fields.ResourceInputFieldView),$body.append(new View(options).render().el)})}}),ResourceEntryView=AbstractResourceEntryView.extend({fields:[{name:"name",title:"Name",view:fields.ResourceNameFieldView},{name:"url",title:"URL",description:"Resources base URL"},{name:"js",title:"JS",description:"Main JavaScript file"},{name:"css",title:"CSS/LESS",description:"List of CSS/LESS files to use for resource",view:fields.ResourceSortableListFieldView},{name:"init",title:"Init",description:"Init instruction for requirejs shim"},{name:"deps",title:"Dependencies",description:"Coma separated values of resources for requirejs shim"},{name:"export",title:"Export",description:"Export vars for requirejs shim"},{name:"conf",title:"Configuration",description:"Configuration in JSON for the widget",view:fields.ResourceTextAreaFieldView}]}),BundleEntryView=AbstractResourceEntryView.extend({fields:[{name:"name",title:"Name",view:fields.ResourceNameFieldView},{name:"resources",title:"Resources",description:"A main resource file to bootstrap bundle or a list of resources to load.",view:fields.BundleResourcesFieldView},{name:"depends",title:"Depends",description:"Bundle this depends on",view:fields.BundleDependsFieldView},{name:"expression",title:"Expression",description:"Conditional expression to decide if this resource will run"},{name:"enabled",title:"Enabled",view:fields.ResourceBoolFieldView},{name:"conditionalcomment",title:"Conditional comment",description:"For Internet Exploder hacks..."},{name:"compile",title:"Does your bundle contain any RequireJS or LESS files?",view:fields.ResourceBoolFieldView},{name:"last_compilation",title:"Last compilation",description:"Date/Time when your bundle was last compiled. Empty, if it was never compiled.",view:fields.ResourceDisplayFieldView},{name:"jscompilation",title:"Compiled JavaScript",description:"Automatically generated path to the compiled JavaScript.",view:fields.ResourceDisplayFieldView},{name:"csscompilation",title:"Compiled CSS",description:"Automatically generated path to the compiled CSS.",view:fields.ResourceDisplayFieldView}]}),RegistryResourceListItem=BaseView.extend({tagName:"li",type:"resource",className:"list-group-item",template:_.template('<a href="#"><%- name %></a> <button class="pull-right plone-btn plone-btn-danger delete plone-btn-xs">Delete</button>'),events:{"click a":"editResource","click button.delete":"deleteClicked"},afterRender:function(){this.$el.attr("data-name",this.options.name),this.$el.addClass(this.type+"-list-item-"+this.options.name)},serializedModel:function(){return $.extend({},{name:this.options.name,view:this.options.registryView},this.options.data)},editResource:function(e){e&&e.preventDefault();var options=$.extend({},this.options,{containerData:this.options.registryView.options.data.resources,parent:this}),resource=new ResourceEntryView(options);this.registryView.showResourceEditor(resource),$("html, body").animate({scrollTop:resource.$el.offset().top},1e3)},deleteClicked:function(e){e.preventDefault(),delete this.options.registryView.options.data.resources[this.options.name],this.options.registryView.dirty=!0,this.options.registryView.render()}}),Builder=function(bundleName,bundleListItem){var self=this;return self.bundleName=bundleName,self.bundleListItem=bundleListItem,self.rview=bundleListItem.options.registryView,self.$results=null,self.$btnClose=null,self.rview.loading.show(),self.modal=new Modal($("<div/>").appendTo(self.rview.$el),{html:_.template('<div id="content"><h1>Bundle Builder</h1><div class="start-info"><p>You are about to build the <span class="label label-primary"><%= name %></span> pattern. </p><p>This may take some a bit of time so we will try to keep you updated on the progress.</p><p> Press the "Build it" button to proceed.</p></div><ul class="list-group hidden"></ul><button class="plone-btn plone-btn-default cancel hidden cancel-build">Close</button><button class="plone-btn plone-btn-primary build">Build it</button></div>',bundleListItem.options),content:null,width:500,buttons:".plone-btn"}),self.modal.on("shown",function(){var $el=self.modal.$modal,$info=$el.find(".start-info");self.$btnClose=$el.find("button.cancel-build");var $btn=$el.find("button.build");$btn.off("click").on("click",function(e){e.preventDefault(),$info.addClass("hidden"),$btn.prop("disabled",!0),self.$results=$el.find(".list-group").removeClass("hidden"),self.buildJS(),self.rview.loading.show()}),self.$btnClose.off("click").on("click",function(){self.modal.hide()})}),self.addResult=function(txt,klass){klass||(klass=""),self.$results.append('<li class="list-group-item '+klass+'">'+txt+"</li>")},self.run=function(){self.modal.show()},self.finished=function(error){var msg="Finished!";error&&(msg="Error in build process"),self.addResult(msg,"list-group-item-warning"),self.$btnClose.removeClass("hidden"),self.rview.loading.hide()},self.buildJS=function(){self.addResult("building javascripts"),$.ajax({url:self.rview.options.data.manageUrl,type:"POST",dataType:"json",data:{action:"js-build-config",bundle:self.bundleName,_authenticator:utils.getAuthenticator()},success:function(data){self._buildJSBundle(data)},error:function(){self.addResult("Error building resources"),self.finished(!0)}})},self._buildCSSBundle=function(config){var $iframe=$('<iframe style="display:none"><html><head></head><body></body></html></iframe>').appendTo("body").on("load",function(){}),iframe=$iframe[0],win=iframe.contentWindow||iframe;win.lessErrorReporting=function(what,error,href){"remove"!==what&&self.addResult("less compilation error on file "+href+": "+error)};var head=$iframe.contents().find("head")[0];_.each(config.less,function(less){var link=document.createElement("link");link.setAttribute("rel","stylesheet/less"),link.setAttribute("type","text/css"),link.setAttribute("href",less),head.appendChild(link)});var script=document.createElement("script");script.setAttribute("type","text/javascript"),script.setAttribute("src",self.rview.options.data.lessConfigUrl),script.onload=function(){script=document.createElement("script"),script.setAttribute("type","text/javascript"),script.setAttribute("src",self.rview.options.data.lessUrl),head.appendChild(script)},head.appendChild(script),self.addResult(config.less.length+" css files to build");var checkFinished=function(){for(var $styles=$('style[type="text/css"][id]',head),i=0;i<$styles.length;i+=1){var $style=$styles.eq(i);if("less:error-message"===$style.attr("id"))return self.addResult("Error compiling less"),self.finished(!0)}if($styles.length===config.less.length){var data={};$styles.each(function(){var $el=$(this);data["data-"+$el.attr("id")]=$el.html()}),$iframe.remove(),$.ajax({url:self.rview.options.data.manageUrl,type:"POST",dataType:"json",data:$.extend(data,{action:"save-less-build",bundle:self.bundleName,_authenticator:utils.getAuthenticator()}),success:function(data){self.rview.options.data.overrides.push(data.filepath),self.rview.tabView.overridesView.render(),self.addResult("finished saving css bundles"),self.finished()},error:function(){self.addResult("Error saving css bundle"),self.finished(!0)}})}else setTimeout(checkFinished,300)};checkFinished()},self.buildCSSBundle=function(){self.addResult("building CSS bundle"),$.ajax({url:self.rview.options.data.manageUrl,type:"POST",dataType:"json",data:{action:"less-build-config",bundle:self.bundleName,_authenticator:utils.getAuthenticator()},success:function(data){self._buildCSSBundle(data)},error:function(){self.addResult("Error building css bundle"),self.finished(!0)}})},self._buildJSBundle=function(config){if(0===config.include.length)return self.addResult("No javascripts to build, skipping"),self.buildCSSBundle();config.out=function(results){$.ajax({url:self.rview.options.data.manageUrl,type:"POST",dataType:"json",data:{action:"save-js-build",bundle:self.bundleName,data:results,_authenticator:utils.getAuthenticator()},success:function(data){self.rview.options.data.overrides.push(data.filepath),self.rview.tabView.overridesView.render()},error:function(){self.addResult("Error building bundle"),self.finished(!0)}})};var $iframe=$('<iframe style="display:none"><html><head></head><body></body></html></iframe').appendTo("body"),iframe=$iframe[0],win=iframe.contentWindow||iframe,head=$iframe.contents().find("head")[0],script=document.createElement("script");script.setAttribute("type","text/javascript"),script.setAttribute("src",self.rview.options.data.rjsUrl),script.onload=function(){win.requirejs.optimize(config,function(combined_files){self.addResult("Saved javascript bundle, Build results: <pre>"+combined_files+"</pre>"),self.buildCSSBundle(),$iframe.remove()})},head.appendChild(script)},self},RegistryBundleListItem=RegistryResourceListItem.extend({type:"bundle",template:_.template('<a href="#"><%- name %></a> <div class="plone-btn-group pull-right"><% if(view.options.data.nonBuildableBundles.indexOf(name) === -1){ %><button class="plone-btn plone-btn-default build plone-btn-xs">Build</button><button class="plone-btn plone-btn-danger delete plone-btn-xs">Delete</button><% } %></div>'),events:$.extend({},RegistryResourceListItem.prototype.events,{"click button.build":"buildClicked"}),editResource:function(e){e&&e.preventDefault();var options=$.extend({},this.options,{containerData:this.options.registryView.options.data.bundles,parent:this}),resource=new BundleEntryView(options);this.registryView.showResourceEditor(resource)},deleteClicked:function(e){e.preventDefault(),delete this.options.registryView.options.data.bundles[this.options.name],this.options.registryView.dirty=!0,this.options.registryView.render()},buildClicked:function(e){e.preventDefault();var self=this,builder=new Builder(self.options.name,self);builder.run()}}),BaseResourcesPane=BaseView.extend({tagName:"div",className:"tab-pane",$form:null,initialize:function(options){var self=this;BaseView.prototype.initialize.apply(self,[options]),self.previousData=self._copyData()},showResourceEditor:function(resource){this.$form.empty().append(resource.render().el)},_copyData:function(){return $.extend(!0,{},this.options.data)},_revertData:function(data){this.options.data=$.extend(!0,{},data)},revertChanges:function(e){e&&e.preventDefault(),confirm("Are you sure you want to cancel? You will lose all changes.")&&(this._revertData(this.previousData),this.render())},afterRender:function(){this.$form=this.$(".form"),this.loading=this.options.tabView.loading}}),RegistryView=BaseResourcesPane.extend({template:_.template('<div class="clearfix"><div class="plone-btn-group pull-right"><button class="plone-btn plone-btn-success save">Save</button><button class="plone-btn plone-btn-default cancel">Cancel</button></div><div class="plone-btn-group pull-right"><button class="plone-btn plone-btn-default add-bundle">Add bundle</button><button class="plone-btn plone-btn-default add-resource">Add resource</button></div></div><div class="row"><div class="checkbox development-mode"><label><input type="checkbox" <% if(data.development){ %> checked="checked" <% } %> > Development Mode(only logged in users)</label></div></div><div class="row"><div class="items col-md-5"><ul class="bundles list-group"><li class="list-group-item list-group-item-warning">Bundles</li></ul><ul class="resources-header list-group"><li class="list-group-item list-group-item-warning">Resources <input class="float-right form-control input-xs" placeholder="Filter..." /></li></ul><ul class="resources list-group"></ul></div><div class="form col-md-7"></div></div>'),events:{"click button.save":"saveClicked","click button.add-resource":"addResourceClicked","click button.add-bundle":"addBundleClicked","click button.cancel":"revertChanges","keyup .resources-header input":"filterResources","change .development-mode input":"developmentModeChanged"},filterTimeout:0,dirty:!1,initialize:function(options){var self=this;BaseResourcesPane.prototype.initialize.apply(self,[options]),$(document).on("resource-data-changed",function(){self.dirty=!0,self.showHideButtons()})},showHideButtons:function(){var val=!0;this.dirty&&(val=!1),this.$("button.save").prop("disabled",val),this.$("button.cancel").prop("disabled",val)},filterResources:function(){var self=this;self.filterTimeout&&clearTimeout(self.filterTimeout),self.filterTimeout=setTimeout(function(){var filterText=self.$(".resources-header input").val().toLowerCase(),$els=self.$(".resources .list-group-item");!filterText||filterText.length<3?$els.removeClass("hidden"):$els.each(function(){var $el=$(this);-1!==$el.find("a").html().toLowerCase().indexOf(filterText)?$el.removeClass("hidden"):$el.addClass("hidden")})},200)},_copyData:function(){return $.extend(!0,{},{bundles:this.options.data.bundles,resources:this.options.data.resources})},_revertData:function(data){this.options.data.bundles=$.extend(!0,{},data.bundles),this.options.data.resources=$.extend(!0,{},data.resources),this.dirty=!1},afterRender:function(){var self=this;self.showHideButtons(),self.$bundles=self.$("ul.bundles"),self.$resources=self.$("ul.resources");var data=self.options.data,bundles=_.sortBy(_.keys(data.bundles),function(v){return v.toLowerCase()});self.items={bundles:{},resources:{}},_.each(bundles,function(resourceName){var item=new RegistryBundleListItem({data:data.bundles[resourceName],name:resourceName,registryView:self});self.items.bundles[resourceName]=item,self.$bundles.append(item.render().el)});var resources=_.sortBy(_.keys(data.resources),function(v){return v.toLowerCase()});return _.each(resources,function(resourceName){var item=new RegistryResourceListItem({data:data.resources[resourceName],name:resourceName,registryView:self});self.items.resources[resourceName]=item,self.$resources.append(item.render().el)}),BaseResourcesPane.prototype.afterRender.apply(self),self},addResourceClicked:function(e){e.preventDefault();var name=utils.generateId("new-resource-");
this.options.data.resources[name]={enabled:!0},this.dirty=!0,this.render(),this.items.resources[name].editResource()},addBundleClicked:function(e){e.preventDefault();var name=utils.generateId("new-resource-");this.options.data.bundles[name]={enabled:!0},this.dirty=!0,this.render(),this.items.bundles[name].editResource()},saveClicked:function(e){var self=this;e.preventDefault(),self.options.tabView.saveData("save-registry",{resources:JSON.stringify(self.options.data.resources),bundles:JSON.stringify(self.options.data.bundles)},function(){self.dirty=!1,self.previousData=self._copyData()})},developmentModeChanged:function(){var self=this,value="false";self.$(".development-mode input")[0].checked&&(value="true"),self.options.tabView.saveData("save-development-mode",{value:value})}});return RegistryView}),define("mockup-patterns-resourceregistry-url/js/patternoptions",["jquery","mockup-ui-url/views/base","underscore","mockup-utils","mockup-patterns-resourceregistry-url/js/fields"],function($,BaseView,_,utils,fields){var PatternOptionsView=BaseView.extend({tagName:"div",className:"tab-pane patternoptions",template:_.template('<div class="clearfix"><div class="btn-group pull-right"><button class="plone-btn plone-btn-default add-pattern">Add pattern</button><button class="plone-btn plone-btn-default save">save</button></div></div><div class="row clearfix"><div class="form col-md-12"></div></div>'),events:{"click .plone-btn.save":"saveClicked","click .plone-btn.add-pattern":"addPattern"},initialize:function(options){BaseView.prototype.initialize.apply(this,[options]),this.loading=this.options.tabView.loading},saveClicked:function(e){e.preventDefault();var self=this;self.options.tabView.saveData("save-pattern-options",{data:JSON.stringify(self.options.data.patternoptions)})},addPattern:function(e){e.preventDefault();var self=this;self.options.data.patternoptions[utils.generateId("new-pattern-")]="",self.render()},inputChanged:function(){var self=this,data={};self.$(".form-group").each(function(){data[$(this).find(".field-name").val()]=$(this).find(".field-value").val()}),self.options.data.patternoptions=data},afterRender:function(){var self=this,settings=self.options.data.patternoptions,$form=self.$(".form");_.each(_.keys(settings),function(name){$form.append(new fields.PatternFieldView({registryData:settings,title:name,name:name,value:settings[name],onChange:function(e,field){try{$.parseJSON(field.$el.find(".field-value").val()),field.$el.removeClass("has-error").removeClass("has-feedback"),field.$(".form-control-feedback").addClass("hidden")}catch(err){field.$el.addClass("has-error").addClass("has-feedback"),field.$(".form-control-feedback").removeClass("hidden")}self.inputChanged()}}).render().el)})}});return PatternOptionsView}),define("mockup-patterns-resourceregistry",["jquery","mockup-patterns-base","underscore","mockup-ui-url/views/base","mockup-utils","mockup-patterns-resourceregistry-url/js/less","mockup-patterns-resourceregistry-url/js/overrides","mockup-patterns-resourceregistry-url/js/registry","mockup-patterns-resourceregistry-url/js/patternoptions"],function($,Base,_,BaseView,utils,LessVariablesView,OverridesView,RegistryView,PatternOptionsView){var TabView=BaseView.extend({tagName:"div",activeTab:"registry",template:_.template('<ul class="main-tabs nav nav-tabs" role="tablist"><li class="registry-btn"><a href="#">Registry</a></li><li class="overrides-btn"><a href="#">Overrides</a></li><li class="lessvariables-btn"><a href="#">Less Variables</a></li><li class="patternoptions-btn"><a href="#">Pattern Options</a></li></div><div class="tab-content" />'),events:{"click .registry-btn a":"hideShow","click .overrides-btn a":"hideShow","click .lessvariables-btn a":"hideShow","click .patternoptions-btn a":"hideShow"},hideShow:function(e){var self=this;void 0!==e&&(e.preventDefault(),self.activeTab=$(e.target).parent()[0].className.replace("-btn","")),self.$(".main-tabs > li").removeClass("active"),self.$content.find(".tab-pane").removeClass("active"),self.tabs[self.activeTab].btn.addClass("active"),self.tabs[self.activeTab].content.addClass("active")},initialize:function(options){var self=this;BaseView.prototype.initialize.apply(self,[options]),self.registryView=new RegistryView({data:options,tabView:self}),self.overridesView=new OverridesView({data:options,tabView:self}),self.lessvariablesView=new LessVariablesView({data:options,tabView:self}),self.patternoptionsView=new PatternOptionsView({data:options,tabView:self}),self.tabs={}},render:function(){var self=this;return self.$el.append(self.template()),self.loading=new utils.Loading,self.$tabs=self.$("ul.main-tabs"),self.$content=self.$(".tab-content"),self.$content.append(self.registryView.render().el),self.$content.append(self.overridesView.render().el),self.$content.append(self.lessvariablesView.render().el),self.$content.append(self.patternoptionsView.render().el),self.tabs={registry:{btn:self.$(".registry-btn"),content:self.registryView.$el},overrides:{btn:self.$(".overrides-btn"),content:self.overridesView.$el},lessvariables:{btn:self.$(".lessvariables-btn"),content:self.lessvariablesView.$el},patternoptions:{btn:self.$(".patternoptions-btn"),content:self.patternoptionsView.$el}},self.hideShow(),self},saveData:function(action,data,onSave,onError){var self=this;self.loading.show(),data||(data={}),data=$.extend({},data,{action:action,_authenticator:utils.getAuthenticator()}),$.ajax({url:self.options.manageUrl,type:"POST",dataType:"json",data:data}).done(function(resp){onSave&&onSave(resp),void 0!==resp.success&&!resp.success&&resp.msg&&alert(resp.msg)}).always(function(){self.loading.hide()}).fail(function(resp){onError?onError(resp):alert("Error processing ajax request for action: "+action)})}}),ResourceRegistry=Base.extend({name:"resourceregistry",trigger:".pat-resourceregistry",defaults:{bundles:{},resources:{},javascripts:{},css:{},overrides:[],manageUrl:null,baseUrl:null,rjsUrl:null,lessvariables:{},patternoptions:{},nonBuildableBundles:["plone-legacy"]},init:function(){var self=this;self.tabs=new TabView(self.options),self.$el.append(self.tabs.render().el)}});return ResourceRegistry}),define("mockup-bundles-resourceregistry",["jquery","mockup-registry","mockup-patterns-resourceregistry"],function($,Registry){$(document).ready(function(){var $registry=$(".pat-resourceregistry");Registry.scan($registry)})}),require(["mockup-bundles-resourceregistry"]);
//# sourceMappingURL=++resource++plone.app.resourceregistry.min.js.map