...
if sort_on and sort_on != 'self':
    ...
    get_sort_attr = lambda x: (callable(getattr(x, sort_on, None)) and
                           getattr(x, sort_on)() or getattr(x, sort_on, None))
...
sublist_len = (list_len % numCols == 0 and list_len / numCols or
                                                    list_len / numCols + 1)
