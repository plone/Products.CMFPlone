...
try:
    ...
    new_context.edit(title=title,
                     ...
                     start_time='%0.2d:%0.2d'
                        % (dt_start.h_24(), dt_start.minute()),
                     startAMPM=dt_start.ampm(),
                     stopAMPM=dt_end.ampm(),
                     stop_time='%0.2d:%0.2d'
                        % (dt_end.h_24(), dt_end.minute()),
                     location=location,
    ...
    raise
# TODO DateTime and contentEdit() has many things that could go wrong - catch
# all.
except:
...
transaction_note('Edited event %s at %s'
                    % (str(new_context.title_or_id()),
